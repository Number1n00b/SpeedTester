...
# Used by Christian Brunette for a Speed recorder program.
#
# Changes made:
# - The shell() function now returns a result set.
# - Multiple print statements removed
# - Multiple System.exit calls removed and replaced with "return None".
...
def shell(signal):
    ...
    config = speedtest.config['client']
    my_results['lat'] = config['lat']
    my_results['long'] = config['lon']
    my_results['isp'] = config['isp']
    my_results['ip'] = config['ip']
    ...
    # my_results['host'] = results.
    # my_results['host_location'] = results['name']
    # my_results['dist'] = results['d']
    # my_results['latency'] = results['latency']
    ...
    my_results['down'] = str((results.download / 1000.0 / 1000.0))
    ...
    #          args.units[0]),
    #         quiet)
    my_results['up'] = str((results.upload / 1000.0 / 1000.0))

    my_results['ping'] = str(results.ping)
